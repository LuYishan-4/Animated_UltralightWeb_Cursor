...
int main(int argc, char** argv)
{
    ...
    if (!UltralightWebCursorM::UserConfig::instance()->load()) {
        qCritical() << "Failed to load user config for Niri version.";
        return 1;
    }
    ...
    if (!effect->initialize(UltralightWebCursorM::UserConfigimp(), UltralightWebCursorM::CursorJSONImp())) {
        qCritical() << "Failed to initialize Ultralight HTML effect for Niri version.";
        return 1;
    }
    ...
    QObject::connect(&timer, &QTimer::timeout, [&]() {
        ...
        if (effect->hasNewFrame()) {
            qDebug() << "Niri effect frame ready:" << effect->width() << "x" << effect->height();
            effect->clearNewFrame();
        }
    });
    timer.start(16);

    qDebug() << "Niri cursor effect running.";
    return app.exec();
}
