...
public:
  ...
  void OnFinishLoading(ultralight::View *view, uint64_t frame_id,
                       bool main_frame,
  ...
  void OnFailLoading(ultralight::View *view, uint64_t frame_id, bool main_frame,
    ...
    if (main_frame) {
      std::cerr << "[Ultralight] load failed " << url.utf8().data() << " "
                << desc.utf8().data() << "\n";
      if (loaded_) {
        *loaded_ = false;
      }
