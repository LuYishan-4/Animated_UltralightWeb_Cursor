...
int main(int argc, char** argv)
{
    ...
    UltralightWebCursorM::g_sdkInitialPath = appDir / "sdk" / "ultralight-free-sdk-1.4.0-linux-x64";
    UltralightWebCursorM::g_htmlInitialPath = appDir / "WebCursor";

    if (!UltralightWebCursorM::UserConfig::instance()->load()) {
        qCritical() << "Failed to load user config for Hyprland version.";
        return 1;
    }
    ...
    if (!effect->initialize(UltralightWebCursorM::UserConfigimp, UltralightWebCursorM::CursorJSONImp)) {
        qCritical() << "Failed to initialize Ultralight HTML effect for Hyprland version.";
        return 1;
    }
    ...
    QObject::connect(&timer, &QTimer::timeout, [&]() {
        ...
        if (effect->hasNewFrame()) {
            qDebug() << "Hyprland effect frame ready:" << effect->width() << "x" << effect->height();
            effect->clearNewFrame();
        }
    });
    timer.start(16);

    qDebug() << "Hyprland cursor effect running.";
    return app.exec();
}
